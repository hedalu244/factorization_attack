...
function showExpanded(expanded) {
    let x = [];
    if (expanded.a !== 0)
        x.push((expanded.a == 1 ? "" : expanded.a) + "x^2");
    if (expanded.b !== 0)
        x.push((expanded.b == 1 ? "" : expanded.b) + "x");
    if (expanded.c !== 0)
        x.push(expanded.c + "");
...
function showFactrized(factrized) {
    let x;
    if (showSolution(factrized.b) == showSolution(factrized.c))
        x = factrized.a + "(x-" + showSolution(factrized.b) + ")^2";
    else
        x = factrized.a + "(x-" + showSolution(factrized.b) + ")" + "(x-" + showSolution(factrized.c) + ")";
